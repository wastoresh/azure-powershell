...
namespace Microsoft.WindowsAzure.Commands.Common.Storage.ResourceModel
{
    ...
    public class AzureStorageBlob : AzureStorageBase
    {
        ...
        public BlobBaseClient BlobBaseClient
        {
            get
            {
                if (privateBlobBaseClient == null)
                {
                    if (this.ICloudBlob != null)
                    {
                        privateBlobBaseClient = GetTrack2BlobClient(this.ICloudBlob, (AzureStorageContext)this.Context, this.privateClientOptions);
                        if (this.VersionId != null && (this.IsCurrentVersion == null || !this.IsCurrentVersion.Value)) // only none current version blob need versionId in Uri
                        { 
                            privateBlobBaseClient.WithVersion(VersionId);
                        }
                    }
                    else if (this.privateBlobClient != null)
                    {
        ...
        // Convert Track1 Blob object to Track 2 blob Client
        public static BlobBaseClient GetTrack2BlobClientWithType(CloudBlob cloubBlob, AzureStorageContext context, BlobClientOptions options = null, global::Azure.Storage.Blobs.Models.BlobType? blobType = null)
        {
            BlobClient blobClient;
            if (cloubBlob.ServiceClient.Credentials.IsToken) //Oauth
            {
