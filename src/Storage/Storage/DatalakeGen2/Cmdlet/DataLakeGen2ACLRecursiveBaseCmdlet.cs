        ...
        protected AccessControlChangeOptions accessControlChangeOptions
        {
            get
            {
                return new AccessControlChangeOptions()
                {
                    ...
                    ContinueOnFailure = !this.StopOnFailure.IsPresent,
        ...
        protected IProgress<Response<AccessControlChanges>> GetProgressHandler()
        {
            ...
            return new ChangeChangeAccessControlPartialResultProgress((setProgress) =>
            {
                if (this.progressRecord != null)
                {
                    ...
                    // TODO: should update this block to "if(setProgress.Value.BatchCounters.FailedChangesCount == 0) {continuationToken = setProgress.Value.ContinuationToken;}" 
                    // Write like this, since the last request without ContinuationToken, XSCL will still set the ContinuationToken of the request before last
                    if ((continuationToken == setProgress.Value.ContinuationToken) && (setProgress.Value.BatchCounters.FailedChangesCount == 0 || !this.StopOnFailure.IsPresent))
                    {
