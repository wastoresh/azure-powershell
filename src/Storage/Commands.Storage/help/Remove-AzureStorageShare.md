...
### ShareName (Default)
```
Remove-AzureStorageShare [-Name] <String> [-IncludeSnapshot] [-Force] [-PassThru] [-Context <IStorageContext>]
...
### Share
```
Remove-AzureStorageShare [-Share] <CloudFileShare> [-IncludeSnapshot] [-Force] [-PassThru]
 [-ServerTimeoutPerRequest <Int32>] [-ClientTimeoutPerRequest <Int32>] [-ConcurrentTaskCount <Int32>] [-WhatIf]
 [-Confirm] [<CommonParameters>]
```
...
### -IncludeSnapshot
Remove the share with its snapshots
If -IncludeSnapshot and -Force options are both not specified, remove the share which has snapshots will fail. ```yaml
Type: SwitchParameter
Parameter Sets: (All)
Aliases: 
