...
namespace Microsoft.WindowsAzure.Commands.Storage.Model.ResourceModel
{
    public class PSCorsRule
    {
        ...
        /// <param name="corsProperties">Cors Rules from XSCL</param>
        /// <returns>PSCorsRule Array</returns>
        public static PSCorsRule[] ParseCorsRules(XSCLOLDProtocol.CorsProperties corsProperties)
        {
            List<PSCorsRule> ruleList = new List<PSCorsRule>();
        ...
        /// <param name="methods">CorsHttpMethods from XSCL</param>
        /// <returns>String Array</returns>
        private static string[] ConvertCorsHttpMethodToString(XSCLOLDProtocol.CorsHttpMethods methods)
        {
            List<string> methodList = new List<string>();

            foreach (XSCLOLDProtocol.CorsHttpMethods methodValue in Enum.GetValues(typeof(XSCLOLDProtocol.CorsHttpMethods)).Cast<XSCLOLDProtocol.CorsHttpMethods>())
            {
                if (methodValue != XSCLOLDProtocol.CorsHttpMethods.None && (methods & methodValue) != 0)
                {
                    methodList.Add(methodValue.ToString());
                }
            }
